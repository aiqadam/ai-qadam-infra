...
status: populated
last_verified_note: 2026-07-19 (T-0117) — Stalwart mail server deployed on pro-data-tech-prod (Compose project `stalwart-mail`: single container), coexisting with Penpot and AiQadam prod; nginx 1.28.3 + Let's Encrypt TLS active for the admin UI; https://mail.aiqadam.org live; SMTP/IMAP/submission serving Stalwart's own internal-ACME-managed TLS. Prior note: 2026-07-13 (T-0111) — AiQadam prod app stack deployed on pro-data-tech-prod (Compose project `aiqadam-prod`: postgres + oidc-stub + api), coexisting with Penpot; nginx 1.28.3 + Let's Encrypt TLS active; https://aiqadam.org live. Prior note: 2026-07-13 (T-0110) — AiQadam QA app stack deployed on pro-data-tech-qa (Compose project `aiqadam-qa`: oidc-stub + api); nginx 1.28.3 + Let's Encrypt TLS active; https://qa-uz.aiqadam.org live.
---
...
| `stalwart-mail-server-1` | `stalwartlabs/stalwart:v0.16` | stalwart-mail | `25`, `465`, `587`, `993` (public); `127.0.0.1:8080` (admin/JMAP UI, loopback only) | Up (healthy) | Self-hosted mail server for `aiqadam.org` — SMTP/IMAP/submission; admin UI proxied by nginx at `https://mail.aiqadam.org`; `Domain` object `dnsManagement`/`certificateManagement` both Automatic (DNS scoped to `publishRecords: {tlsa:true}`); test mailbox `test@aiqadam.org` provisioned; deployed 2026-07-19 (T-0117) |
...
- **Status (2026-07-11, T-0109; 2026-07-13, T-0111; 2026-07-19, T-0117):** nginx **1.28.3** — `active` and `enabled`. Package: `nginx 1.28.3-2ubuntu1.6` (Ubuntu apt). Vhosts: `/etc/nginx/sites-available/penpot.aiqadam.org` (symlinked to `sites-enabled/penpot.aiqadam.org`), `/etc/nginx/sites-available/aiqadam.org` (symlinked to `sites-enabled/aiqadam.org`, added T-0111), and `/etc/nginx/sites-available/mail.aiqadam.org` (symlinked to `sites-enabled/mail.aiqadam.org`, added T-0117). Config (Penpot): HTTP→HTTPS redirect on port 80; HTTPS on port 443 with `client_max_body_size 367001600`; WebSocket proxy for `/ws/notifications` and `/mcp/ws`; SSE proxy for `/mcp/stream`; general proxy for `/` → `http://localhost:9001/`. Config (AiQadam prod): HTTP→HTTPS redirect on port 80; HTTPS on port 443 proxying `/` → `http://127.0.0.1:3115/`; bare apex only, no `www`. Config (Stalwart mail admin UI): HTTP→HTTPS redirect on port 80; HTTPS on port 443 proxying `/` → `http://127.0.0.1:8080` with WebSocket upgrade headers.
