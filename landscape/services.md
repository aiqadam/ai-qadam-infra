...
status: populated
last_verified_note: 2026-07-21 (T-0121) — Stalwart mail admin UI nginx-restricted to loopback-only (external HTTPS returns 403); AllowedIp 172.19.0.1 and 172.19.0.0/16 added to Stalwart config store; monitoring cron installed on pro-data-tech-prod. Prior note: 2026-07-19 (T-0117) — Stalwart mail server deployed on pro-data-tech-prod (Compose project `stalwart-mail`: single container), coexisting with Penpot and AiQadam prod; nginx 1.28.3 + Let's Encrypt TLS active for the admin UI; https://mail.aiqadam.org live; SMTP/IMAP/submission serving Stalwart's own internal-ACME-managed TLS. Prior note: 2026-07-13 (T-0111) — AiQadam prod app stack deployed on pro-data-tech-prod (Compose project `aiqadam-prod`: postgres + oidc-stub + api), coexisting with Penpot; nginx 1.28.3 + Let's Encrypt TLS active; https://aiqadam.org live. Prior note: 2026-07-13 (T-0110) — AiQadam QA app stack deployed on pro-data-tech-qa (Compose project `aiqadam-qa`: oidc-stub + api); nginx 1.28.3 + Let's Encrypt TLS active; https://qa-uz.aiqadam.org live.
---
...
| `aiqadam-prod` | `/opt/apps/aiqadam-prod/deploy/docker-compose.prod.yml` | 4 (`aiqadam-prod-postgres-1`, `aiqadam-prod-oidc-stub-1`, `aiqadam-prod-api-1`, `aiqadam-prod-web-next-1`) — original 3 deployed 2026-07-13 (T-0111); `web-next` added 2026-07-18 by separate, out-of-band work, discovered 2026-07-19 via a T-0117 no-regression check, user-confirmed expected — see [`hosts/pro-data-tech-prod.md`](hosts/pro-data-tech-prod.md) |
| `stalwart-mail` | `/opt/stalwart-mail/` (Compose file managing the single `stalwart-mail-server-1` container) | 1 (`stalwart-mail-server-1`) — deployed 2026-07-19 (T-0117), self-hosted mail server for `aiqadam.org`, replacing the dead third-party `mail.aiqadam.org` host — see [`hosts/pro-data-tech-prod.md`](hosts/pro-data-tech-prod.md#stalwart-mail) |
...
| `stalwart-mail-server-1` | `stalwartlabs/stalwart:v0.16` (running v0.16.13) | stalwart-mail | `25`, `465`, `587`, `993` (public); `127.0.0.1:8080` (admin/JMAP UI, loopback only) | Up (healthy) | Self-hosted mail server for `aiqadam.org` — SMTP/IMAP/submission; admin UI proxied by nginx at `https://mail.aiqadam.org` (**access restricted to loopback** — external HTTPS returns 403, operator access via SSH port-forward, T-0121 2026-07-21); `Domain` object `dnsManagement`/`certificateManagement` both Automatic (DNS scoped to `publishRecords: {tlsa:true}`); `AllowedIp` for `172.19.0.1` (id `i9yv13qeaaqa`) and `172.19.0.0/16` (id `i9yv3mloabaa`) permanent entries (T-0121); mailboxes: test@, admin@, postmaster@aiqadam.org; deployed 2026-07-19 (T-0117), hardened 2026-07-21 (T-0121) |
...
- **Stalwart mail server:** Compose project `stalwart-mail` (single container). Deployed 2026-07-19 by [T-0117](../tasks/T-0117-install-mail-server-aiqadam.md). Public endpoints `https://mail.aiqadam.org` (admin UI, nginx + Let's Encrypt TLS) and SMTP/IMAP/submission on 25/465/587/993 (Stalwart's own internal-ACME-managed TLS). See [`hosts/pro-data-tech-prod.md`](hosts/pro-data-tech-prod.md#stalwart-mail) for full detail.

### nginx

- **Status (2026-07-11, T-0109; 2026-07-13, T-0111; 2026-07-19, T-0117; 2026-07-21, T-0121):** nginx **1.28.3** — `active` and `enabled`. Package: `nginx 1.28.3-2ubuntu1.6` (Ubuntu apt). Vhosts: `/etc/nginx/sites-available/penpot.aiqadam.org` (symlinked to `sites-enabled/penpot.aiqadam.org`), `/etc/nginx/sites-available/aiqadam.org` (symlinked to `sites-enabled/aiqadam.org`, added T-0111), and `/etc/nginx/sites-available/mail.aiqadam.org` (symlinked to `sites-enabled/mail.aiqadam.org`, added T-0117). Config (Penpot): HTTP→HTTPS redirect on port 80; HTTPS on port 443 with `client_max_body_size 367001600`; WebSocket proxy for `/ws/notifications` and `/mcp/ws`; SSE proxy for `/mcp/stream`; general proxy for `/` → `http://localhost:9001/`. Config (AiQadam prod): HTTP→HTTPS redirect on port 80; HTTPS on port 443 proxying `/` → `http://127.0.0.1:3115/`; bare apex only, no `www`. Config (Stalwart mail admin UI): HTTP→HTTPS redirect on port 80; HTTPS on port 443 proxying `/` → `http://127.0.0.1:8080` with WebSocket upgrade headers; `X-Forwarded-For` and `X-Real-IP` headers present; **IP restriction added T-0121 (2026-07-21): `allow 127.0.0.1; deny all;` in `location /` — external HTTPS returns HTTP 403, operators use SSH port-forward**.

### certbot

- **Status (2026-07-11, T-0109; 2026-07-13, T-0111; 2026-07-19, T-0117):** certbot **4.0.0** + python3-certbot-nginx **4.0.0** — installed. `certbot.timer` active and enabled (auto-renewal). TLS certificate for `penpot.aiqadam.org`: `/etc/letsencrypt/live/penpot.aiqadam.org/` (ECDSA, expires 2026-10-09, intermediate CA `YE1`/Let's Encrypt, HTTP-01 challenge). TLS certificate for `aiqadam.org`: `/etc/letsencrypt/live/aiqadam.org/` (ECDSA, expires 2026-10-11, T-0111). TLS certificate for `mail.aiqadam.org` (admin UI only): `/etc/letsencrypt/live/mail.aiqadam.org/` (ECDSA, expires 2026-10-17, T-0117 — reused from an orphaned executor-attempt-1 cert). Renewal configs: `/etc/letsencrypt/renewal/penpot.aiqadam.org.conf`, `/etc/letsencrypt/renewal/aiqadam.org.conf`, and `/etc/letsencrypt/renewal/mail.aiqadam.org.conf`. Note: Stalwart's SMTP/IMAP/submission TLS (25/465/587/993) is managed independently via Stalwart's own internal ACME (DNS-01), not by certbot — see [`hosts/pro-data-tech-prod.md`](hosts/pro-data-tech-prod.md#stalwart-mail).
...
- Root crontab: `*/5 * * * * /usr/local/bin/mail-health-check.sh` (mail reachability monitor — checks HTTPS+SMTP 25/587/993 external reachability, logs to syslog via `logger` and to `/var/log/mail-health-check.log`; installed 2026-07-21, T-0121).
...
| 2026-07-19 | `2026-07-19-install-mail-server-aiqadam-001` | T-0117: Stalwart mail server deployed on pro-data-tech-prod — Compose project `stalwart-mail` (single container `stalwart-mail-server-1`, image `stalwartlabs/stalwart:v0.16`); `Domain aiqadam.org` `dnsManagement`/`certificateManagement` both Automatic (DNS scoped to `publishRecords: {tlsa:true}`); nginx vhost + Let's Encrypt TLS live at `https://mail.aiqadam.org` (admin UI); SMTP/IMAP/submission serving Stalwart's own internal-ACME cert; test mailbox `test@aiqadam.org` created; Cloudflare `aiqadam.org` mail records cut over from the dead third-party host; Penpot and AiQadam-prod confirmed unregressed throughout. T-0117 closed done/succeeded (9 executor attempts). |
| 2026-07-21 | `2026-07-21-harden-stalwart-auto-ban-001` | T-0121 done — nginx `mail.aiqadam.org` vhost restricted to loopback-only (`allow 127.0.0.1; deny all;`, external HTTPS 403); Stalwart `AllowedIp` entries for `172.19.0.1` and `172.19.0.0/16` created permanently (JMAP `x:AllowedIp/set`); monitoring cron `/usr/local/bin/mail-health-check.sh` every 5 min; `proxyTrustedNetworks` investigated (PROXY protocol binary, not X-Forwarded-For) and reverted; Stalwart v0.16.13; all mail ports reachable; Penpot and AiQadam prod unregressed. |
