...
See [`../services.md`](../services.md) for the canonical per-host table. High-level: **Penpot 2.16 deployed (T-0108, 2026-07-11) — 7 Docker Compose containers running under project `penpot`; MCP enabled (`penpot-mcp` running); nginx 1.28.3 + Let's Encrypt TLS active (T-0109, 2026-07-11) — https://penpot.aiqadam.org live.** **AiQadam prod app stack deployed (T-0111, 2026-07-13) — 4 Docker Compose containers running under project `aiqadam-prod`; nginx vhost + Let's Encrypt TLS live — https://aiqadam.org live.** **Stalwart mail server deployed (T-0117, 2026-07-19) — 1 Docker Compose container running under project `stalwart-mail`; nginx vhost + Let's Encrypt TLS at https://mail.aiqadam.org (admin UI restricted to loopback — external HTTPS returns 403, operator access via SSH port-forward required, T-0121 2026-07-21); SMTP/IMAP/submission serving their own internal-ACME-managed TLS cert. Roundcube webmail deployed (T-0122, 2026-07-23) — 1 Docker Compose container running under project `roundcube`; nginx vhost + Let's Encrypt TLS at https://webmail.aiqadam.org; IMAP via IMAPS 993 (Stalwart does not bind 143 internally).** Docker CE 29.6.1 (T-0106).
...
Deployed by run [`2026-07-23-deploy-roundcube-webmail-001`](../../runs/2026-07-23-deploy-roundcube-webmail-001/) (T-0122, 2026-07-23). Roundcube 1.6.17 webmail UI for `@aiqadam.org` users, running as a single-container Docker Compose stack under project name `roundcube` at `/opt/roundcube/`, joined to the `stalwart-mail_default` Docker network.

- **Compose directory:** `/opt/roundcube/`
- **Compose file:** `/opt/roundcube/docker-compose.yml`
- **Env file:** `/opt/roundcube/.env` (mode 600 — `ROUNDCUBEMAIL_CIPHER_KEY` session encryption key; see [`../secrets-inventory.md`](../secrets-inventory.md) key `roundcube-cipher-key`)
- **Image:** `roundcube/roundcubemail:1.6.17-apache`
- **Container:** `roundcube-1`, `Up`, port `127.0.0.1:8888→80/tcp`
- **Docker networks:** `stalwart-mail_default` (external — joined so Roundcube can reach Stalwart and its source IP falls within the `172.19.0.0/16` `AllowedIp` range, preventing auto-ban) + `roundcube_internal` (internal)
- **Docker volumes:** `roundcube_roundcube_db` (SQLite DB at `/var/roundcube/db/roundcube.db` inside container), `roundcube_roundcube_config`, `roundcube_roundcube_temp`
- **IMAP backend:** `ssl://mail.aiqadam.org:993` (IMAPS — Stalwart does **not** bind port 143 internally; confirmed via `/proc/net/tcp6`; connection verified from inside the container via `openssl s_client -connect mail.aiqadam.org:993`, `Verify return code: 0 (ok)`)
- **SMTP backend:** `tls://mail.aiqadam.org:587` (STARTTLS submission)
- **nginx vhost:** `/etc/nginx/sites-available/webmail.aiqadam.org` (symlinked to `/etc/nginx/sites-enabled/webmail.aiqadam.org`) — HTTP→HTTPS redirect on port 80; HTTPS on port 443 proxying `/` → `http://127.0.0.1:8888`; no IP restrictions (public-facing user endpoint)
- **TLS cert:** `/etc/letsencrypt/live/webmail.aiqadam.org/` (ECDSA, expires 2026-10-21, auto-renewing via the same `certbot.timer` active on this host); renewal config at `/etc/letsencrypt/renewal/webmail.aiqadam.org.conf`
- **Pre-nginx backup:** `/etc/nginx/sites-available/webmail.aiqadam.org.pre-prod.bak`
- **Public URL:** `https://webmail.aiqadam.org/` — HTTP 200 + Roundcube login page confirmed from external workstation (step-07 PASS, 2026-07-23)
- **Live browser login test:** pending user action (outside automated verification scope)
...
- **Vhost (Roundcube webmail, T-0122, 2026-07-23):** `/etc/nginx/sites-available/webmail.aiqadam.org` (symlinked to `/etc/nginx/sites-enabled/webmail.aiqadam.org`) — proxies to `http://127.0.0.1:8888`; no IP restrictions; public-facing user endpoint
...
- **Config (Roundcube webmail):** HTTP→HTTPS redirect on port 80 (certbot-managed); HTTPS on port 443 proxying `/` → `http://127.0.0.1:8888`; no IP restrictions
...
- **TLS (Roundcube webmail):** Let's Encrypt via certbot 4.0.0; cert at `/etc/letsencrypt/live/webmail.aiqadam.org/` (ECDSA, expires 2026-10-21); auto-renewal via the same `certbot.timer` (active)
- **Access URLs:** `https://penpot.aiqadam.org`, `https://aiqadam.org`, `https://mail.aiqadam.org`, and `https://webmail.aiqadam.org` — all HTTP 200/302 confirmed from external workstation (step-07 PASS, T-0109, T-0111, T-0117, T-0122 respectively)
...
| 2026-07-23 | `2026-07-23-deploy-roundcube-webmail-001` | T-0122 done — Roundcube 1.6.17-apache deployed as Docker Compose project `roundcube` at `/opt/roundcube/` (single container `roundcube-1`, image `roundcube/roundcubemail:1.6.17-apache`, port `127.0.0.1:8888→80/tcp`, joined to `stalwart-mail_default` + `roundcube_internal` networks); SQLite DB on Docker volume `roundcube_roundcube_db`; IMAP via IMAPS `ssl://mail.aiqadam.org:993` (Stalwart does not bind 143 internally — confirmed via `/proc/net/tcp6`); nginx vhost `/etc/nginx/sites-available/webmail.aiqadam.org` created and symlinked; Let's Encrypt cert `/etc/letsencrypt/live/webmail.aiqadam.org/` (ECDSA, expires 2026-10-21, auto-renewal via certbot.timer); Cloudflare A record `webmail.aiqadam.org` → `95.46.211.224` created (record ID `d44ce1ab6990cf47848148634809463d`, proxied=false); `https://webmail.aiqadam.org/` HTTP 200 + Roundcube login page confirmed from external workstation; Penpot, Stalwart, and AiQadam prod unregressed. |
